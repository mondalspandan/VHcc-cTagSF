	OUTPUTDIR=/nfs/dust/cms/user/spmondal/ctag_condor/190923_2017_DY/
	...
	INPFILE=$3
        ...
        tmp_dir=$(mktemp -d)
        cp -r ../DYJetSelection.py ../nuSolutions.py ../scalefactors* $tmp_dir
        ...
        python DYJetSelection.py ${INPFILE}
        rc=$?
        if [[ $rc != 0 ]]
        then
	...
	until cp -vf ${OUTPUTNAME} ${OUTPUTDIR}${SAMPNAME}"/outTree_"${CONDOR_PROCESS_ID}"_NEW.root"; do
